...
4 and more dimensions because we can't plot the data because it would be four dimensions. PCA takes 4 or more dimensions of data and make a 2D PCA plot, similar individuals cluster together. PCA can also show us which variables are the most valuable for clustering the data. Our analysis using principal component analysis is based on Singular Value Decomposition (SVD).
...
Our analysis was carried out in a single line of code using the `sklearn` library, which abstracts all the mathematical calculations for the principle components analysis' transformation of data. With this library, we only need to provide the number of principal components we want our model to have {cite}`Kumar:2021tg`. The principle components analysis watch the following steps at a high level {cite}`Starmer:2018wp,Kumar:2021tg`:

- Standardize the dataset (prerequisite for every principle components analysis).
...
- Calculate the `Eigenvalues` and `Eigenvectors` to be able to identify principal components:
    ...
    - Draw a line that is perpendicular to principle component 1 and also goes through the origin. This line is the `principal component 2`.
...
The StatQuest PCA step-by-step video {cite}`Starmer:2018wp` provides a very good visual representation of the different steps involved in the Principle Component Analysis.

(pca-results)=
The results of the principle components analysis in our case are displayed in {numref}`pca-table-headf`. The total variation around the principle component 1 is equal to {glue:}`pca_pc1_vp` and the one around principle component 2 to {glue:}`pca_pc2_vp`.

```{glue:figure} pca_table_headf
:name: "pca-table-headf"
:figwidth: 400px

First five rows of the generated Principle Components Analysis' results
```
(plot-the-results)=
### Plot the results
